...
impl Vector {
    pub fn crossed(&self, other: &Self) -> Result<Self, ()> {
...
mod tests {
    ...
    fn question_4() {
        println!(
            "{:?}",
            Vector(vec![2.0, -5.0, 1.0]).dotted(&Vector(vec![1.0, 3.0, 4.0]))
        );
        println!(
            "{:?}",
            Vector(vec![2.0, -5.0, 1.0]).crossed(&Vector(vec![1.0, 3.0, 4.0]))
        );
    }
}
